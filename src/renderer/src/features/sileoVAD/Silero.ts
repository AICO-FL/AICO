...
class OnnxWrapper {
  ...
  private sample_rates: number[] = [8000, 16000]
  ...
  async ready(): Promise<void> {
    ...
    if (!this.session) {
      throw new Error('OnnxWrapper: Session initialization failed')
    }
  }
  ...
  async call(x: number[][], sr: number): Promise<number[][]> {
    ...
    if (sr === 8000 || sr === 16000) {
      ...
      if (!this.session) {
        throw new Error('OnnxWrapper: Session is not initialized')
      }
      ...
      const out = Array(outputShape[0])
        .fill(0)
        .map((_, i) => Array.from(outputData.slice(i * outputShape[1], (i + 1) * outputShape[1])))
  ...
  close(): void {
    if (this.session) {
      ...
      console.warn('OnnxWrapper: Attempted to close a session that was not initialized')
    }
  }
}
